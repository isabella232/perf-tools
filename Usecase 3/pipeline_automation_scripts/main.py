...
def run_pipeline(api_endpoint, credential_token, pipeline_name,payload):
    ...
    #use this URL for Batch pipelines
    url = api_endpoint + "/v3/namespaces/default/apps/{}/workflows/DataPipelineWorkflow/start".format(pipeline_name)

    #use this URL for realtime pipelines
    #url = api_endpoint + "/v3/namespaces/default/apps/{}/spark/DataStreamsSparkStreaming/start".format(pipeline_name)
...
def runtime_argument(pipeline_name):
    ...
    '''
    #load runtime argument json file.        
    file = open(runtime_arg_json_path).read()
    json_file = json.loads(file)
    runtime_arg_list=[]
    runtime_arg={}
    runtime_arg['system.profile.name']= compute_profile
    runtime_arg.update(json_file)
    runtime_arg_list.append(json.dumps(runtime_arg))
    return runtime_arg_list
print(run_pipeline.__doc__)

def main(region, instance_name, compute_profile, pipeline_name, pipeline_json_path,runtime_arg_json_path):
...
if __name__ == "__main__":
    if len(sys.argv) <= 6:
        sys.exit('region, instance_name, compute_profile, pipeline_name, pipeline_json_path,runtime_arg_json_path')
    region = sys.argv[1]
    instance_name = sys.argv[2]
    compute_profile = sys.argv[3]
    pipeline_name = sys.argv[4]
    pipeline_json_path = sys.argv[5]
    runtime_arg_json_path = sys.argv[6]
    main(region, instance_name, compute_profile, pipeline_name, pipeline_json_path, runtime_arg_json_path)
